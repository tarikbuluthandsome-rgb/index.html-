...
const { User }      = require('../lib/models.js');
...
module.exports = async function handler(req, res) {
    ...
    if (action === 'login') {
        ...
        return res.status(200).json({ token, user: { _id: user._id, username: user.username, email: user.email, role: user.role } });
    }
    ...
    if (action === 'register') {
        ...
        const newUser = await User.create({ username, email: email.toLowerCase(), password: hashed, fullName, status: 'pending' });
        return res.status(201).json({ success: true, userId: newUser._id, message: 'Kayıt talebiniz alındı, onay bekleniyor.' });
    }
    ...
    if (action === 'approve-user') {
        ...
        const user = await User.findByIdAndUpdate(userId, { status: 'active', approvedDate: new Date() }, { new: true });
    ...
    if (action === 'ban-user') {
        ...
        const user = await User.findByIdAndUpdate(userId, { status: 'banned', banReason: reason || 'Belirtilmedi', banDate: new Date() }, { new: true });
        if (!user) return res.status(404).json({ error: 'Kullanıcı bulunamadı.' });
        return res.status(200).json({ success: true });
    }
    ...
    if (action === 'change-master') {
        ...
        return res.status(200).json({ success: true, hash: newHash });
    }
